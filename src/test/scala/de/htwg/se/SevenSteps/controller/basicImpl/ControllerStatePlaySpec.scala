...
class ControllerStatePlaySpec extends WordSpec {
  ...
  "A Controller in game phase play" should {
    "switch between different Players and can't undo that" in {
      val c = before()
      c.state.players.getCurPlayer.name should be("Hans1")
      c.nextPlayer().isSuccess should be(true)
      c.state.players.getCurPlayer.name should be("Hans2")
      c.nextPlayer().isSuccess should be(true)
      c.state.players.getCurPlayer.name should be("Hans3")
      c.nextPlayer().isSuccess should be(true)
      c.state.players.getCurPlayer.name should be("Hans1")
    ...
    "allow a player to set a stone on the grid" in {
      ...
      c.state.grid.cell(0, 0).get.height should be(1)
    ...
    "give on Transition Prepare -> Play the first player colors with 7 stones" in {
      val c = before()
      c.state.grid.getColors should be(List('a', 'b'))
      c.state.players.getCurPlayer.getStoneNumber should be(7)
      c.state.players(1).getStoneNumber should be(0)
    }
    "set the first stone everywhere on height 0" in {
      val c = before()
      c.state.players = c.state.players.setAllStonesTo(999)
      c.state.grid.getHeights should be(List(0, 0, 0, 0))
      c.setStone(0, 0).isSuccess should be(true)
      c.state.grid.getHeights should be(List(1, 0, 0, 0))
      c.undo().isSuccess should be(true)
      c.setStone(0, 1).isSuccess should be(true)
      c.state.grid.getHeights should be(List(0, 1, 0, 0))
      c.undo().isSuccess should be(true)
      c.setStone(1, 0).isSuccess should be(true)
      c.state.grid.getHeights should be(List(0, 0, 1, 0))
      c.undo().isSuccess should be(true)
      c.setStone(1, 1).isSuccess should be(true)
      c.state.grid.getHeights should be(List(0, 0, 0, 1))
    }
    "set the second Stone neighboring to the first stone" in {
      ...
      c.setStone(0, 0).isSuccess should be(true)
      c.setStone(0, 1).isSuccess should be(true)
      c.state.grid.getHeights should be(List(1, 1, 0, 0))
      c.undo().isSuccess should be(true)
      c.setStone(1, 0).isSuccess should be(true)
      c.state.grid.getHeights should be(List(1, 0, 1, 0))
      c.undo().isSuccess should be(true)
    ...
    "not set in one turn a grid cell twice" in {
      ...
      c.setStone(1, 1).isSuccess should be(true)
      c.setStone(1, 0).isSuccess should be(true)
      c.state.grid.getHeights should be(List(1, 1, 1, 1))
      ...
      c.state.grid.getHeights should be(List(1, 1, 1, 1))
      c.undo().isSuccess should be(true)
      c.state.grid.getHeights should be(List(1, 1, 0, 1))
      c.setStone(1, 0).isSuccess should be(true)
    }
    "only allow players to set stones on current height +0 or +1" in {
      ...
      c.setStone(0, 0).isSuccess should be(true)
      c.setStone(1, 0).isSuccess should be(false)
      c.state.grid.getHeights should be(List(2, 1, 0, 0))
    }
    "only allow players to set stones when they have stones from that color" in {
      val c = before()
      c.state.players = c.state.players.setAllStonesTo(1)
      c.setStone(0, 0).isSuccess should be(true)
      c.setStone(0, 1).isSuccess should be(false)
    }
    "can check if the game is finished (there are no stones any more)" in {
      ...
      c.state.players = c.state.players.setAllStonesTo(0)
      while (c.state.bag.get().isDefined) {}
    ...
    "checks on command next if the game is finished" in {
      ...
      while (c.state.bag.get().isDefined) {}
      c.nextPlayer().isSuccess should be(true)
      c.state.gameState.isInstanceOf[Finish] should be(true)
      c.getWinningPlayer should be(c.getCurPlayer)
      c.isGameEnd should be(true)
    }
    "selects the winning player with the most stones" in {
      val c = before()
      c.state.players = c.state.players.setAllStonesTo(2)
      c.setStone(0, 0).isSuccess should be(true)
      c.setStone(1, 0).isSuccess should be(true)
      val winner = c.state.players.getCurPlayer
      c.nextPlayer().isSuccess should be(true)
      c.setStone(1, 1).isSuccess should be(true)
      // delete all stones
      c.state.players = c.state.players.setAllStonesTo(0)
    ...
    "can check deadlock situation when grid is full" in {
      val c = before("a")
      c.state.players = c.state.players.setAllStonesTo(3)
    ...
    "can check deadlock situation when grit is not full" in {
      ...
      c.state.players = c.state.players.setAllStonesTo(1)
      while (c.state.bag.get().isDefined) {}
      c.state.players = c.state.players.updateCurPlayer(c.getCurPlayer.incColor('b', -1))
      c.setStone(0, 0).isSuccess should be(true)
      c.nextPlayer().isSuccess should be(true)
      c.isDeadlock should be(false)
      c.state.players = c.state.players.updateCurPlayer(c.getCurPlayer.incColor('b', -1))
